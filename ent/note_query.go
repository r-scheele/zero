...
type NoteQuery struct {
	...
	withOwner   *UserQuery
	withLikes   *NoteLikeQuery
	withReposts *NoteRepostQuery
	withFKs     bool
...
func (nq *NoteQuery) QueryLikes() *NoteLikeQuery {
	query := (&NoteLikeClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NoteQuery) QueryReposts() *NoteRepostQuery {
	query := (&NoteRepostClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := nq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (nq *NoteQuery) Clone() *NoteQuery {
	...
	return &NoteQuery{
		...
		withLikes:   nq.withLikes.Clone(),
		withReposts: nq.withReposts.Clone(),
...
func (nq *NoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Note, error) {
	...
	if query := nq.withLikes; query != nil {
		if err := nq.loadLikes(ctx, query, nodes,
			func(n *Note) { n.Edges.Likes = []*NoteLike{} },
			func(n *Note, e *NoteLike) { n.Edges.Likes = append(n.Edges.Likes, e) }); err != nil {
			return nil, err
		}
	}
	if query := nq.withReposts; query != nil {
		if err := nq.loadReposts(ctx, query, nodes,
			func(n *Note) { n.Edges.Reposts = []*NoteRepost{} },
			func(n *Note, e *NoteRepost) { n.Edges.Reposts = append(n.Edges.Reposts, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NoteQuery) loadLikes(ctx context.Context, query *NoteLikeQuery, nodes []*Note, init func(*Note), assign func(*Note, *NoteLike)) error {
	...
	query.Where(predicate.NoteLike(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(note.LikesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.note_likes
		if fk == nil {
			return fmt.Errorf(`foreign-key "note_likes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "note_likes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (nq *NoteQuery) loadReposts(ctx context.Context, query *NoteRepostQuery, nodes []*Note, init func(*Note), assign func(*Note, *NoteRepost)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Note)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.NoteRepost(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(note.RepostsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.note_reposts
		if fk == nil {
			return fmt.Errorf(`foreign-key "note_reposts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "note_reposts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
