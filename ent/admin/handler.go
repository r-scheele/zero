...
func (h *Handler) UserCreate(ctx echo.Context) error {
	...
	op.SetPhoneNumber(payload.PhoneNumber)
	...
	if payload.VerificationCode != nil {
		op.SetVerificationCode(*payload.VerificationCode)
	}
	op.SetAdmin(payload.Admin)
	if payload.RegistrationMethod != nil {
		op.SetRegistrationMethod(*payload.RegistrationMethod)
	}
	if payload.ProfilePicture != nil {
		op.SetProfilePicture(*payload.ProfilePicture)
	}
	op.SetDarkMode(payload.DarkMode)
	if payload.Bio != nil {
		op.SetBio(*payload.Bio)
	}
	op.SetEmailNotifications(payload.EmailNotifications)
...
func (h *Handler) UserUpdate(ctx echo.Context, id int) error {
	...
	op.SetName(payload.Name)
	op.SetPhoneNumber(payload.PhoneNumber)
	op.SetNillableEmail(payload.Email)
	if payload.Password != nil {
		op.SetPassword(*payload.Password)
	}
	op.SetVerified(payload.Verified)
	op.SetNillableVerificationCode(payload.VerificationCode)
	op.SetAdmin(payload.Admin)
	if payload.RegistrationMethod == nil {
		var empty user.RegistrationMethod
		op.SetRegistrationMethod(empty)
	} else {
		op.SetRegistrationMethod(*payload.RegistrationMethod)
	}
	op.SetNillableProfilePicture(payload.ProfilePicture)
	op.SetDarkMode(payload.DarkMode)
	op.SetNillableBio(payload.Bio)
	op.SetEmailNotifications(payload.EmailNotifications)
	op.SetSmsNotifications(payload.SmsNotifications)
	op.SetIsActive(payload.IsActive)
	...
	for i := 0; i <= len(res)-1; i++ {
		list.Entities = append(list.Entities, EntityValues{
			...
			Values: []string{
				...
				func() string {
					if res[i].VerificationCode != nil {
						return *res[i].VerificationCode
					}
					return ""
				}(),
				fmt.Sprint(res[i].Admin),
				fmt.Sprint(res[i].RegistrationMethod),
				func() string {
					if res[i].ProfilePicture != nil {
						return *res[i].ProfilePicture
					}
					return ""
				}(),
				fmt.Sprint(res[i].DarkMode),
				func() string {
					if res[i].Bio != nil {
						return *res[i].Bio
					}
				...
				fmt.Sprint(res[i].EmailNotifications),
				fmt.Sprint(res[i].SmsNotifications),
				fmt.Sprint(res[i].IsActive),
				func() string {
					if res[i].LastLogin != nil {
						return res[i].LastLogin.Format(h.Config.TimeFormat)
					}
					return ""
				}(),
				res[i].CreatedAt.Format(h.Config.TimeFormat),
				func() string {
					if res[i].UpdatedAt != nil {
						return res[i].UpdatedAt.Format(h.Config.TimeFormat)
					}
					return ""
				}(),
			},
...
func (h *Handler) UserGet(ctx echo.Context, id int) (url.Values, error) {
	...
	v.Set("phone_number", entity.PhoneNumber)
	v.Set("email", func() string {
		if entity.Email != nil {
			return *entity.Email
		}
		return ""
	...
	v.Set("verification_code", func() string {
		if entity.VerificationCode != nil {
			return *entity.VerificationCode
		}
		return ""
	}())
	v.Set("admin", fmt.Sprint(entity.Admin))
	v.Set("registration_method", fmt.Sprint(entity.RegistrationMethod))
	v.Set("profile_picture", func() string {
		if entity.ProfilePicture != nil {
			return *entity.ProfilePicture
		}
		return ""
	}())
	v.Set("dark_mode", fmt.Sprint(entity.DarkMode))
	v.Set("bio", func() string {
		if entity.Bio != nil {
			return *entity.Bio
		}
		return ""
	}())
	v.Set("email_notifications", fmt.Sprint(entity.EmailNotifications))
	v.Set("sms_notifications", fmt.Sprint(entity.SmsNotifications))
	v.Set("is_active", fmt.Sprint(entity.IsActive))
	v.Set("last_login", func() string {
		if entity.LastLogin != nil {
			return entity.LastLogin.Format(dateTimeFormat)
		}
		return ""
	}())
