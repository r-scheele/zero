...
func (h *Files) Routes(g *echo.Group) {
	files := g.Group("/files", middleware.RequireAuthentication, middleware.RequireVerification)
	files.GET("", h.Page).Name = routenames.Files
	files.POST("", h.Submit).Name = routenames.FilesSubmit
}
