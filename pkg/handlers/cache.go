...
func (h *Cache) Routes(g *echo.Group) {
	cache := g.Group("/cache", middleware.RequireAuthentication, middleware.RequireVerification)
	cache.GET("", h.Page).Name = routenames.Cache
	cache.POST("", h.Submit).Name = routenames.CacheSubmit
}
