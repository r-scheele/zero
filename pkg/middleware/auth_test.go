...
func TestRequireAdmin(t *testing.T) {
	...
	nonAdminUser, err := tests.CreateUser(c.ORM)
	require.NoError(t, err)

	// Explicitly ensure the user is not an admin but is verified
	err = c.ORM.User.UpdateOneID(nonAdminUser.ID).SetAdmin(false).SetVerified(true).Exec(goctx.Background())
	require.NoError(t, err)

	// Login as a non-admin
	err = c.Auth.Login(ctx, nonAdminUser.ID)
	require.NoError(t, err)
	_ = tests.ExecuteMiddleware(ctx, LoadAuthenticatedUser(c.Auth))
